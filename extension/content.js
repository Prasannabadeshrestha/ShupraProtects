...
// Helper for UTF-8 safe Base64 encoding (FIXED: Ensures btoa works with all Unicode chars)
function utf8ToBase64(str) {
  try {
    // 1. Encode the string to a UTF-8 sequence (represented as bytes)
    const utf8Bytes = new TextEncoder().encode(str);
    
    // 2. Convert the byte array to a "binary string" (Latin-1 compatible for btoa)
    const binaryString = String.fromCharCode.apply(null, utf8Bytes);
    
    // 3. Perform the standard Base64 encoding
    return btoa(binaryString);
  } catch (e) {
    console.error("Error in UTF-8 Base64 encoding:", e);
    // Fallback in case TextEncoder is unavailable or another error occurs
    // This uses a different path but is still safer than direct btoa(str)
...
function startAutoScan() {
  const observer = new MutationObserver(() => {
    try {
      ...
      // Add unique ID (FIXED: Uses utf8ToBase64)
